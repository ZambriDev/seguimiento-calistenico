...
export default function SeguimientoCalistenicoApp() {
  ...
  // Mini tests runtime (no rompen UI)
  useEffect(() => {
    try {
      const p = parseReps("3x10-15");
      console.assert(p && p.sets === 3 && p.max === 15, "parseReps ok");
      console.assert(Array.isArray(WEEK) && WEEK.length === 7, "WEEK length ok");
    } catch {}
  }, []);
      ...
      <div className="max-w-screen-sm mx-auto p-4 pb-24">
        ...
        {!showWeek ? (
          <div className="mt-4 space-y-4">
            ...
            <DayCard dayIdx={idxToday} state={state} setState={setState} />
            <DietCard />
          </div>
        ) : (
          <div className="mt-4 grid grid-cols-1 gap-4">
            {WEEK.map((_, i) => (
              <div key={i} className={i === idxToday ? "ring-2 ring-green-500 rounded-2xl" : ""}>
                <DayCard dayIdx={i} state={state} setState={setState} />
              </div>
            ))}
          </div>
        )}
